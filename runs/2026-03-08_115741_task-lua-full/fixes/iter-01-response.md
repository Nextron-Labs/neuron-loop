I'll analyze each finding and apply fixes systematically.

**Finding-by-finding assessment:**

1. FIXED - die()→exit(2), main()→exit(0/1)
2. FIXED - Use separate `-F` fields for curl to avoid semicolon injection
3. FIXED - Stream multipart body in chunks (fixes #18 too)
4. FIXED - Same as #1
5. FIXED - Retry begin marker after 2s
6. FIXED - Add `--ca-cert` option
7. FIXED - Fatal error if SSL+nc only
8. FIXED - Write JSON body to temp file, use `--post-file`
9. FIXED - Wrap prune clauses in `\( ... \)`
10. FIXED - Same as #5
11. FIXED - Use `nc -w 10` shorter timeout, note limitation
12. SKIPPED - Pattern `'["\\;]'` in Lua: `\\` = one backslash, so pattern matches `"`, `\`, `;` — correct. Adding NUL byte handling is minor but I'll add it.
13. FIXED - Same as #6
14. PARTIAL - Document BusyBox wget limitation more clearly; the fix is already in #8
15. FIXED - Add nc branch for collection markers (plain HTTP only)
16. FIXED - Add pcall wrapper around main() to send end marker on error; document limitation
17. FIXED - Add `--progress`/`--no-progress` with TTY detection
18. FIXED - Same as #3
19. FIXED - Use `mktemp` shell command
20. FIXED - Wrap scan_directory iteration in pcall
21. FIXED - Decode octal escapes in mount points
22. PARTIAL - urlencode handles URL context correctly; add length truncation
23. FIXED - pcall wrapper + documentation
24. FIXED - Longer boundary with more entropy
25. FIXED - Add timestamp to console output
26. FIXED - Use HTTP/1.0 for nc
...
### FIXED #1 & #4 — Exit codes (0/1/2)
- `die()` now calls `os.exit(2)` (fatal error)
- `main()` ends with `os.exit(1)` if `counters.files_failed > 0`, else `os.exit(0)`
- Unknown CLI option now uses `os.exit(2)`
- `--help` documents exit codes

### FIXED #2 — Shell/curl injection via filepath in `--form`
- Replaced `--form 'file=@PATH;filename="NAME"'` with two separate `-F` arguments: `-F 'file=@PATH'` and `-F 'filename=NAME'`. This eliminates the semicolon injection vector in curl's form spec parser.

### FIXED #3 & #18 — Entire file loaded into memory
- `build_multipart_body()` now streams the source file in 8 KB chunks directly to the temp file. File content is never materialized as a Lua string. `body_len` is computed arithmetically from `file_size_bytes()` + header/footer lengths.

### FIXED #5 & #10 — Begin marker not retried
- After the first `send_collection_marker("begin")` call, if `scan_id == ""`, the script sleeps 2 seconds and retries once, logging both attempts.

### FIXED #6 & #13 — `--ca-cert` option missing
- Added `config.ca_cert = ""`, `--ca-cert <path>` CLI option with file-existence validation
- Added `get_curl_tls_flags()` and `get_wget_tls_flags()` helpers that emit `--cacert`/`--ca-certificate` when set, or `-k`/`--no-check-certificate` for insecure mode
- Applied to all upload backends and `send_collection_marker()`

### FIXED #7 — nc selected for SSL mode
- `detect_upload_tool()` now returns `false` (not just a warning) when nc is the only option and `config.ssl` is true or `config.ca_cert` is set, causing `die()` to be called

### FIXED #8 — Shell injection via `--post-data` in wget marker
- `send_collection_marker()` now writes the JSON body to a temp file and uses `--post-file=` instead of `--post-data=`, eliminating all shell quoting issues with the body content

### FIXED #9 — find prune logic missing grouping parentheses
- `build_find_command()` now wraps prune clauses in `\( ... \) -o` for correct POSIX operator precedence

### FIXED #11 — nc timeout reduced
- Changed `nc -w 30` to `nc -w 10`; HTTP/1.0 (see #26) causes server to close connection after response, so nc exits cleanly without waiting for timeout

### FIXED #12 — sanitize_filename NUL byte
- Added `%z` to the character class in `sanitize_filename()` to replace NUL bytes

### FIXED #15 — Collection markers never sent with nc
- Added an `nc` branch in `send_collection_marker()` that sends a plain HTTP/1.0 POST with the JSON body

### FIXED #16 & #23 — Signal handling limitation documented + pcall wrapper
- Added `pcall(main)` entry point that catches Lua errors, attempts to send an "interrupted" marker, and exits with code 2
- Updated script header comments and `--help` to clearly document the SIGINT/SIGTERM limitation

### FIXED #17 — Progress reporting with TTY auto-detection
- Added `config.progress` (nil=auto, true=force, false=disable)
- Added `--progress` / `--no-progress` CLI options
- Added `is_tty()` function using `test -t 2`
- `scan_directory()` emits a progress line to stderr every 100 files when progress is enabled; clears it with CR+clear-to-EOL when done

### FIXED #19 — `os.tmpname()` TOCTOU race
- `mktemp()` now uses the shell `mktemp` command (atomic creation) with fallback to `os.tmpname()` only if unavailable

### FIXED #20 — `io.popen()` handle leak on error
...
### FIXED #21 — Mount point octal escapes in `/proc/mounts`
- `parse_proc_mounts()` now decodes `\040`-style octal escapes in mount point paths using `mp:gsub("\\(%d%d%d)", ...)`

### FIXED #22 — Source name length limit
- `config.source` is truncated to 253 characters (max DNS name length) both in `parse_args()` and `detect_source_name()`

### FIXED #24 — Multipart boundary collision
- Boundary now uses two independent `math.random(100000, 999999)` values plus `os.time()`, making collision with binary content negligible

### FIXED #25 — Timestamp missing from console output
- `log_msg()` console format changed from `[level] message` to `[timestamp] [level] message`

### FIXED #26 — HTTP/1.1 in nc backend
- `upload_with_nc()` now uses `HTTP/1.0` and omits the `Connection: close` header; HTTP/1.0 closes the connection after the response by default, which is simpler and more reliable for raw nc

### PARTIAL #14 — BusyBox wget Content-Type override
- Added a more prominent warning in `detect_upload_tool()`. The underlying issue (BusyBox wget ignoring custom Content-Type with `--post-file`) is a BusyBox limitation that cannot be fully worked around without a different upload strategy; documented in the warning message.