I'll analyze each finding carefully and apply fixes. Let me go through them:

**Finding 1 (CRITICAL)**: Shell injection via filepath in curl --form → FIXED: Use multipart body builder like wget/nc backends
**Finding 2 (CRITICAL)**: body_len TOCTOU → FIXED: Compute body_len from actual temp file size after writing
**Finding 3 (MEDIUM)**: Boundary/header consistency → FIXED: Restructure so boundary is finalized before header/footer
**Finding 4 (HIGH)**: exec_ok detection → FIXED: More defensive handling
**Finding 5 (HIGH)**: nc double shell-quoting → FIXED: Don't pre-quote host/port inside inner command
**Finding 6 (HIGH)**: nc not supported in send_collection_marker → FIXED: Add nc branch
**Finding 7 (HIGH)**: max-age off by a day → FIXED: Use -mmin for minute granularity
**Finding 8 (HIGH)**: nc pipe vs redirect → FIXED: Use redirect instead of pipe
**Finding 9 (HIGH)**: wget marker single-quote issue → FIXED: Use shell_quote properly
**Finding 10 (HIGH)**: find prune portability → FIXED: Add Lua-level prefix check as second defense
**Finding 11 (HIGH)**: False positive → SKIPPED
**Finding 12 (HIGH)**: False positive → SKIPPED
**Finding 13 (HIGH)**: Duplicate Content-Length → FIXED: Remove explicit Content-Length header, use actual temp file size
**Finding 14/15 (HIGH/MEDIUM)**: Mount point space encoding → FIXED: Decode \040 etc.
**Finding 16 (MEDIUM)**: timeout dependency → FIXED: Probe for timeout, fall back gracefully
**Finding 17 (MEDIUM)**: --quiet suppresses errors → FIXED: Always show error/warn
**Finding 18 (MEDIUM)**: Backoff calculation → FIXED: Use `^` operator
**Finding 19 (MEDIUM)**: False positive → SKIPPED
**Finding 20 (MEDIUM)**: HTTP/2 pattern → PARTIAL: Add HTTP/2 pattern
**Finding 21 (MEDIUM)**: False positive → SKIPPED
**Finding 22 (MEDIUM)**: Double-read memory → FIXED: Skip boundary check for small files (boundary is sufficiently unique with os.time + random)
**Finding 23 (MEDIUM)**: False positive → SKIPPED
**Finding 24 (MEDIUM)**: Temp file accumulation → FIXED: Reuse resp_file per submit_file call
**Finding 25 (LOW)**: sanitize_filename pattern comment → FIXED: Add comment
**Finding 26 (LOW)**: nc not used for markers → FIXED (same as Finding 6)
**Finding 27 (LOW)**: False positive → SKIPPED
**Finding 28 (LOW)**: math.randomseed → FIXED: Add call in main()
...
### FIXED — Finding 1 (CRITICAL): Shell injection via filepath in curl --form
Rewrote `upload_with_curl` to use the same `build_multipart_body` approach as the wget/nc backends. The file is streamed to a temp file and passed via `--data-binary @tmpfile` with an explicit `Content-Type` header. The raw filepath is never embedded inside a shell-quoted form string. The `resp_file` parameter is now passed in from `submit_file` (see Finding 24).

### FIXED — Finding 2 (CRITICAL): body_len TOCTOU / incorrect Content-Length
After streaming all content to the temp file, `out:seek("end")` measures the actual bytes written. This value is returned as `actual_body_len` and used for `Content-Length`, eliminating the TOCTOU skew between the initial `file_size_bytes()` call and the actual streamed content.

### FIXED — Finding 3 (MEDIUM): Boundary/header consistency
Restructured `build_multipart_body` so the boundary is fully finalized (including collision-check regeneration) **before** `header` and `footer` strings are constructed. Added a clear comment. The large-file path now also uses the same long-boundary format from the start.

### FIXED — Finding 4 (HIGH): exec_ok defensive handling
`exec_ok` now captures all three return values (`a, b, c`) and handles: `true` (Lua 5.2+ success), `false`/`nil` with optional `b=="exit", c==0` check, and numeric `a==0` (Lua 5.1). This covers patched runtimes and edge cases.

### FIXED — Finding 5 (HIGH): nc double shell-quoting of host/port
The nc upload command no longer wraps host and port in `shell_quote()` inside an inner `sh -c` string. Instead, the command is built directly without `sh -c`, using