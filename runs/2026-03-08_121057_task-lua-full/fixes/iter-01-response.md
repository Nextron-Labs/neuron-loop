I'll analyze each finding and apply fixes systematically. Let me work through them:

**Finding 1/4**: Exit codes - FIXED (die→exit(2), main→exit(0/1))
**Finding 2**: Shell injection in curl --form - PARTIAL (document limitation, add NUL strip)
**Finding 3/8**: Memory in build_multipart_body - FIXED (streaming write)
**Finding 5**: Begin-marker retry - FIXED
**Finding 6/12**: --ca-cert support - FIXED
**Finding 7/25**: Signal handling - FIXED (documented + shell wrapper comment)
**Finding 9**: wget --post-data injection - FIXED (use temp file)
**Finding 10**: find prune logic - FIXED (add /* pattern)
**Finding 11**: nc timeout - FIXED (timeout wrapper + temp file)
**Finding 13**: sanitize_filename NUL bytes - FIXED
**Finding 14**: popen handle leak - FIXED (pcall wrapper)
**Finding 15**: os.tmpname race - FIXED (prefer mktemp shell command)
**Finding 16**: marker ignores exec failure - FIXED
**Finding 17**: stdout banner/summary - FIXED (respect --quiet)
**Finding 18**: json_escape %c overlap - FIXED (single-pass)
**Finding 19**: nc URL parsing nil checks - FIXED
**Finding 20**: wget_is_busybox detection - FIXED
**Finding 21**: syslog per-message subprocess - FIXED (filter debug from syslog)
**Finding 22**: boundary not checked against content - FIXED
**Finding 23**: positional args silently ignored - FIXED
**Finding 24**: TOCTOU race - SKIPPED (documented as known limitation)
**Finding 26**: wget missing Content-Length - FIXED
**Finding 27**: mktemp race - FIXED (merged with finding 15)
**Finding 28**: console log missing timestamp - FIXED
**Finding 29**: globals - PARTIAL (add standalone comment, key vars localized where safe)
...
**Finding 1 & 4 (CRITICAL/HIGH) — Exit codes 0/1/2**
- `die()` now calls `os.exit(2)` (fatal error)
- Unknown CLI option handler uses `os.exit(2)`
- `main()` ends with `os.exit(1)` if `counters.files_failed > 0`, else `os.exit(0)`

**Finding 2 (CRITICAL) — Shell injection in curl --form (PARTIAL)**
- Added NUL byte and control char stripping to `sanitize_filename()` (the primary risk vector)
- Added a code comment documenting the known limitation for paths with literal newlines
- The `shell_quote()` wrapping of the full `--form` value is correct; the `safe_name` portion is now fully sanitized

**Finding 3 & 8 (CRITICAL/HIGH) — Memory: entire file loaded into Lua**
- `build_multipart_body()` now streams the source file in 32KB chunks directly to the temp file
- Header and footer are written separately; no full-body string is ever materialized in memory
- `body_len` is computed from `file_size_bytes()` + `#header` + `#footer`

**Finding 5 (HIGH) — Begin-marker retry**
- After the first `send_collection_marker("begin")` call, if `scan_id == ""`, sleep 2s and retry once

**Finding 6 & 12 (HIGH) — `--ca-cert` support**
- Added `config.ca_cert = ""`
- Added `--ca-cert <path>` to `parse_args()` and `print_help()`
- Added `validate_config()` check that the file exists
- Replaced `get_curl_insecure_flag()` with `get_curl_tls_flags()` (returns `--cacert <path>` or `-k`)
- Added `get_wget_tls_flags()` (returns `--ca-certificate=<path>` or `--no-check-certificate`)
- All upload backends and `send_collection_marker()` use these new helpers

**Finding 7 & 25 (HIGH/MEDIUM) — Signal handling**
- Added a detailed shell wrapper example in the script header comment
- Added note in `print_help()` about the Lua 5.1 limitation

**Finding 9 (HIGH) — wget --post-data injection**
- `send_collection_marker()` now writes the JSON body to a temp file and uses `--post-file=` instead of `--post-data=`
- Also switched curl to `--data-binary @<file>` for consistency

**Finding 10 (HIGH) — find prune logic**
- `build_find_command()` now emits both `-path X -prune` and `-path X/* -prune` for each excluded path, covering both the directory itself and its contents across BusyBox and GNU find

**Finding 11 (HIGH) — nc timeout and unbounded response**
- `upload_with_nc()` now redirects nc output to a temp file using `timeout 35 sh -c "..."` wrapper
- Response is read with `resp_f:read(4096)` (max 4KB) instead of `exec_capture` reading unbounded output

**Finding 13 (HIGH) — sanitize_filename NUL bytes**
- Added `r:gsub("%z", "_")` for NUL bytes and `r:gsub("%c", "_")` for remaining control characters

**Finding 14 (HIGH) — popen handle leak**
- `scan_directory()` wraps the `handle:lines()` iteration in `pcall`; `handle:close()` is always called after the pcall regardless of success/failure

**Finding 15 & 27 (MEDIUM/LOW) — os.tmpname() race**
- `mktemp()` now first tries the shell `mktemp` utility via `io.popen`; falls back to `os.tmpname()` only if unavailable

**Finding 16 (MEDIUM) — marker ignores exec failure**
- `send_collection_marker()` now uses `exec_ok(cmd)` and logs a warning on failure before skipping response parsing

**Finding 17 (MEDIUM) — unconditional stdout output**
- `print_banner()` returns immediately if `config.quiet` is true
- Final summary `io.write()` is gated by `not config.quiet`

**Finding 18 (MEDIUM) — json_escape %c overlap**
- Rewrote `json_escape()` as a single-pass byte-by-byte function; handles DEL (0x7F) correctly; no double-processing risk

**Finding 19 (MEDIUM) — nc URL parsing nil checks**
- Added nil checks for `hostpath`, `hostport`, and `host` with descriptive error messages
- Added `validate_config()` check that `config.server` contains no slashes

**Finding 20 (MEDIUM) — wget_is_busybox() detection**
- Now positively identifies GNU wget by `"gnu wget"` substring; treats unknown variants as BusyBox (conservative)

**Finding 21 (MEDIUM) — syslog per-message subprocess**
- `log_msg()` skips syslog for `level == "debug"` to prevent thousands of subprocess spawns during debug scans

**Finding 22 (MEDIUM) — boundary not checked against file content**
- For files ≤ 64KB: reads content and regenerates boundary (up to 5 times) if collision found
- For files > 64KB: uses a longer boundary (two `math.random` suffixes) to reduce collision probability without reading the whole file

**Finding 23 (MEDIUM) — positional args silently ignored**
- Added `else` branch in `parse_args()` that writes a warning to stderr for unexpected non-flag arguments

**Finding 26 (MEDIUM) — wget missing Content-Length**
- `upload_with_wget()` now adds `--header="Content-Length: <body_len>"` using the value already computed by `build_multipart_body()`

**Finding 28 (LOW) — console log missing timestamp**
- `log_msg()` console output changed from `[level] message` to `[timestamp] [level] message`

### SKIPPED

**Finding 24 (MEDIUM) — TOCTOU race between size check and upload**
- This is an inherent limitation of scanning live filesystems. Documented in the script header comment. Fixing it would require re-reading the file size inside the upload function after already reading the file, adding complexity for minimal benefit on embedded targets.

**Finding 29 (LOW) — all variables are global**
- PARTIAL: Added a prominent comment in the script header that this script is designed to be run standalone, not `require()`'d. Full localization of all functions and module-level variables would be a significant refactor that risks breaking the Lua 5.1 global environment assumptions and is out of scope for a minimal fix.