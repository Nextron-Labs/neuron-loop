...
[
  {
    ...
    "title": "Lua character class `[\"\\\\;]` does not escape backslash correctly — backslash is never sanitized",
    "description": "The pattern `'[\"\\\\;]'` in Lua source is parsed as the character class containing `\"`, `\\`, and `;`. However, inside a Lua long-bracket or single-quoted string the sequence `\\\\` is two literal backslashes, so the character class is `[\"\\\\ ;]` which actually contains `\"`, `\\`, `\\`, `;` — the double-backslash inside `[]` is redundant but harmless in Lua 5.1. Wait — the real issue is different: the string is `'[\"\\\\;]'`. In a Lua single-quoted string there are NO escape sequences, so `\\\\` is literally four characters: two backslashes. The character class therefore is `["\\;]` which matches `"`, `\`, `\`, `;` — the second `\` is redundant but the class still works. Actually the critical bug is that `sanitize_filename` is called on the *filename* component only (basename), but the full `filepath` is passed directly into shell commands (e.g., `shell_quote(filepath)` in log messages and `submit_file`). The filename used in the multipart `Content-Disposition` header goes through `sanitize_filename`, but the actual file path passed to `io.open` and `shell_quote` is the raw path from `find` output. A filename containing a double-quote that survives into the multipart header could break the HTTP header parsing on the server side. More critically: `safe_name` is embedded inside a double-quoted string in the Content-Disposition header (`filename=\"%s\"`) — if `sanitize_filename` fails to strip `\"` the header is malformed.",
    "impact": "Malformed Content-Disposition header if a filename contains a double-quote that is not properly escaped. On some servers this causes the upload to be rejected or misrouted.",
    "suggestion": "After `sanitize_filename`, also apply `json_escape`-style escaping or simply replace any remaining `\"` with `_` before embedding in the header string. Alternatively use percent-encoding for the filename parameter per RFC 5987.",
  ...
  },
  {
    ...
    "title": "Content-Length header sent to curl is redundant and may cause duplicate-header rejection",
    ...
    "impact": "Uploads via curl will be rejected by RFC-compliant servers or security-aware proxies/WAFs.",
    "suggestion": "Remove the explicit `-H 'Content-Length: ...'` from the curl command. curl handles this automatically when `--data-binary @file` is used.",
  ...
  },
  {
    ...
    "title": "Boundary collision check reads file into memory — can OOM on embedded systems for files up to 64 KB",
    ...
    "impact": "On very constrained devices, memory spikes during boundary collision check. Predictable boundary values could theoretically allow a crafted file to always trigger a collision (DoS of the upload).",
    "suggestion": "For the collision check, read the file in chunks and search for the boundary incrementally rather than loading the whole file. For boundary uniqueness, combine `os.time()`, `math.random`, and a per-call counter.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F4",
    ...
    "title": "HTTP/2 response pattern is matched twice with overlapping regexes — one pattern is unreachable",
    ...
    "impact": "Low direct impact, but the dead code indicates the response-parsing logic was not carefully reviewed, and edge-case HTTP responses from proxies may be misclassified as failures.",
    "suggestion": "Simplify to: `if resp:match('HTTP/%d[%.%d]* 2%d%d') then return true end`. Remove the redundant third pattern.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F5",
    ...
    "title": "nc marker Content-Length uses `#body` (UTF-8 character count) not byte count",
    ...
    "impact": "Minor resource waste (one extra temp file per marker when using nc). On systems with very limited /tmp space this could matter.",
    "suggestion": "In the nc branch, skip writing `body_file` and write `body` directly into the request file (as already done). Move the `body_file` creation inside the curl/wget branches only.",
    "false_positive_risk": "low"
  },
  {
    "id": "F6",
    ...
    "title": "`io.popen` handle closed after `pcall` but `handle:lines()` iterator may leave handle open on error",
    ...
    "impact": "Potential hang on `handle:close()` after an error in the scan loop on BusyBox systems.",
    "suggestion": "Before closing, drain remaining output or use `os.execute` with a temp file instead of `io.popen` for the find command. Alternatively, document this as a known limitation.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F7",
    ...
    "title": "`-mmin` with value 0 when `max_age=0` causes `find` to return no files or behave unexpectedly",
    ...
    "impact": "Setting `--max-age 0` silently scans nothing, which is confusing and could be mistaken for a successful empty scan.",
    "suggestion": "Either reject `max_age = 0` in `validate_config` with a clear error, or treat 0 as \"no age filter\" (omit the `-mmin` clause entirely).",
    "false_positive_risk": "low"
  },
  {
    "id": "F8",
    "severity": "HIGH",
    "location": "exec_ok",
    "title": "`exec_ok` mishandles Lua 5.2+ three-return-value case — false negatives possible",
    ...
    "impact": "Low direct impact currently, but the confusing logic could lead to incorrect modifications in the future.",
    "suggestion": "Simplify: `if type(a) == 'boolean' then return a end; if type(a) == 'number' then return a == 0 end; return false`",
  ...
  },
  {
    ...
    "title": "scan_id is not URL-encoded before appending to api_endpoint",
    ...
    "impact": "Truncated scan_id causes the server to not associate uploaded files with the correct scan session.",
    "suggestion": "Use a more robust JSON extraction that handles `\\\"` inside the value, or validate that the extracted scan_id matches an expected format (e.g., UUID pattern) before use.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F10",
    ...
    "title": "nc backend does not handle chunked Transfer-Encoding or HTTP/1.1 persistent connections — response body may be empty",
    ...
    "impact": "False upload failures when server sends 100-Continue responses.",
    "suggestion": "Add `Expect:` header set to empty string in the nc request to suppress 100-Continue: `req_f:write('Expect:\\r\\n')`. Or handle 100-Continue by skipping to the next HTTP response in the buffer.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F11",
    ...
    "title": "Mountpoint with trailing slash causes incorrect prefix matching in `is_excluded_path`",
    ...
    "impact": "Network filesystem mountpoints with trailing slashes are not excluded, causing the collector to attempt scanning network filesystems.",
    "suggestion": "Strip trailing slashes from mountpoints after decoding: `mp = mp:gsub('/*$', '')`",
    "false_positive_risk": "low"
  },
  {
    "id": "F12",
    ...
    "title": "Temp file for multipart body is registered in `temp_files` via `mktemp()` but body_file is never explicitly cleaned up between retries",
    ...
    "impact": "Temp filesystem exhaustion on embedded systems during large scans with retries.",
    "suggestion": "In `submit_file`, call `build_multipart_body` once before the retry loop and reuse the same body file across retries (the file content doesn't change between retries). Or explicitly remove the body temp file after each failed attempt.",
    "false_positive_risk": "low"
  },
  {
    "id": "F13",
    ...
    "title": "`wget_is_busybox()` is called even when wget is not found, and its result is not cached",
    ...
    "impact": "Minor: extra subprocess spawning. On slow embedded systems this adds latency.",
    "suggestion": "Cache the result of `wget_is_busybox()` in a module-level variable, similar to `has_timeout_cmd`.",
    "false_positive_risk": "low"
  },
  {
    "id": "F14",
    ...
    "title": "Unused `body_len` variable from `build_multipart_body` — wrong function called for markers",
    ...
    "impact": "Minor resource waste on nc-only systems.",
    "suggestion": "Create `body_file` only in the curl/wget branches.",
    "false_positive_risk": "low"
  },
  {
    "id": "F15",
    ...
    "title": "Begin marker retry appends scan_id to endpoint even when scan_id is empty string from first attempt",
    ...
    "impact": "No impact — logic is correct.",
  ...
  },
  {
    ...
    "title": "curl error file (`resp_file .. '.err'`) may collide with another temp file if resp_file path ends in a pattern that creates a valid existing path",
    ...
    "impact": "Low probability collision in concurrent execution scenarios. Could cause one instance to read another's error output.",
    "suggestion": "Use `mktemp()` for the error file as well, rather than deriving it from `resp_file`.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F17",
    ...
    "title": "Server hostname validation only checks for `/` but not for other injection characters",
    ...
    "impact": "Malformed HTTP requests if server hostname contains CRLF sequences. Not exploitable for injection since the attacker controls the config, but could cause confusing failures.",
    "suggestion": "Add validation: `if config.server:match('[%c%s]') then die('Server hostname contains invalid characters') end`",
    "false_positive_risk": "low"
  },
  {
    "id": "F18",
    "severity": "MEDIUM",
    "location": "build_find_command",
    "title": "Prune clauses use `-o` (OR) without wrapping in parentheses — logical operator precedence may cause incorrect pruning",
    ...
    "impact": "On some BusyBox find implementations, excluded paths may not be properly pruned, causing the collector to scan /proc, /sys, etc., leading to hangs or errors.",
    "suggestion": "Wrap the prune clauses in escaped parentheses: `prune_str = '\\\\( ' .. table.concat(prune_parts, ' -o ') .. ' \\\\) -o '`",
    "false_positive_risk": "medium"
  },
  {
    "id": "F19",
    ...
    "title": "Syslog `logger` command is called with `os.execute` (not `exec_ok`) — failures are silently ignored",
    ...
    "impact": "Silent syslog failures. Low impact since syslog is optional.",
    "suggestion": "Either use `exec_ok` and log a one-time warning if it fails, or document that syslog failures are intentionally ignored.",
    "false_positive_risk": "low"
  },
  {
    "id": "F20",
    "severity": "LOW",
    "location": "main",
    "title": "`detect_upload_tool()` called twice — once in the dry-run block and once implicitly via `detect_timeout_cmd` probe",
    ...
    "impact": "Minor: extra subprocess spawning on startup. Negligible on most systems.",
    "suggestion": "Call `detect_upload_tool()` once before the dry-run check and store the result.",
    "false_positive_risk": "low"
  }
]
```