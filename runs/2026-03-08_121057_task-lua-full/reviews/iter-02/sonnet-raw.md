...
[
  {
    ...
    "impact": "An attacker who can create a file with a crafted name (containing single quotes) in a scanned directory could achieve shell command injection when the collector processes that file.",
    "suggestion": "Use `shell_quote(filepath)` separately and pass it as a curl variable, or write the form spec to a temp file and use `--config`. Alternatively: `local form_val = 'file=@' .. filepath .. ';filename=\"' .. safe_name .. '\"'` then pass `shell_quote(form_val)` — but this still embeds the raw filepath. The safest fix is to use curl's `-F` with a file reference written to a curl config file, or to use the multipart body builder (like wget/nc backends do) and pass `--data-binary @tmpfile` with explicit Content-Type.",
  ...
  },
  {
    ...
    "title": "body_len calculation uses pre-read src_size but file may be re-read with different size",
    ...
    "impact": "Incorrect Content-Length causes upload failures or server-side corruption. On active log files or frequently written files this is a real scenario.",
    "suggestion": "After streaming the file to the temp file, seek to end of the temp file to get the actual body size, then update body_len. Or: compute body_len from the actual temp file size after writing. Also, fix the boundary regeneration logic to regenerate `header` and `footer` after changing the boundary.",
    "false_positive_risk": "low"
  },
  {
    "id": "F3",
    ...
    "title": "Boundary regenerated but header/footer not rebuilt with new boundary",
    ...
    "impact": "Code is confusing and fragile; a future edit could easily break the ordering.",
    "suggestion": "Restructure so boundary finalization happens before header/footer construction, with a clear comment.",
  ...
  },
  {
    ...
    "title": "nc command uses cat piped to nc, which may not work on all BusyBox nc variants; also no SSL support warning is insufficient",
    ...
    "impact": "All uploads via nc will appear to fail even when the server accepted them, causing files to be retried and counted as failed, inflating `files_failed` and returning exit code 1.",
    "suggestion": "After writing the request, use `nc -w 30 host port < req_file > resp_file` (redirect, not pipe) which is more portable. Also consider using `timeout` around the entire nc invocation. The current code already uses timeout 35 but the inner nc -w 30 may not honor it on all implementations.",
  ...
  },
  {
    ...
    "title": "wget marker command uses single-quoted --header flag which breaks on some BusyBox wget",
    ...
    "impact": "On edge-case shells, the Content-Type header may not be set correctly, causing the server to reject the JSON marker body.",
    "suggestion": "Use `shell_quote('Content-Type: application/json')` and pass it via `--header=` with proper quoting: `'--header=' .. shell_quote('Content-Type: application/json')`.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F6",
    ...
    "title": "find prune logic is incorrect — -prune without -o -false causes find to print pruned dirs",
    ...
    "impact": "On BusyBox systems (the primary target), excluded paths like /proc/* may not be properly excluded, causing find to descend into /proc and potentially hang or produce enormous output.",
    "suggestion": "Test with BusyBox find. Consider using `-path 'X' -prune -o -path 'X/*' -prune` but also add a fallback: after getting each file path from find, do a prefix check in Lua against the exclude list as a second line of defense.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F7",
    ...
    "title": "io.popen handle not closed on early return paths",
  ...
  },
  {
    ...
    "title": "scan_id appended to api_endpoint inside the directory loop causes double-appending on second iteration",
  ...
  },
  {
    ...
    "title": "wget --post-file sends multipart body but wget may add its own Content-Length incorrectly",
    ...
    "impact": "Upload failures on active filesystems; potential HTTP protocol errors.",
    "suggestion": "Remove the explicit `Content-Length` header from the wget command and rely on wget's automatic calculation, OR compute body_len from the actual temp file size after writing (which eliminates the TOCTOU issue for the temp file itself).",
    "false_positive_risk": "low"
  },
  {
    "id": "F10",
    ...
    "impact": "Network or special filesystems mounted at paths containing spaces will not be excluded, potentially causing the collector to scan them.",
    "suggestion": "After extracting `mp`, decode `\\040` to space: `mp = mp:gsub('\\\\040', ' ')`. Also handle `\\011` (tab) and `\\012` (newline) for completeness.",
    "false_positive_risk": "low"
  },
  {
    "id": "F11",
    ...
    "title": "Error and warn messages suppressed by --quiet flag",
    ...
    "impact": "Operators using --quiet in automated pipelines will miss error messages, making debugging impossible. Exit code alone is insufficient to diagnose failures.",
    "suggestion": "Change the condition to: `if not config.quiet or level == 'error' or level == 'warn' then` to always show errors and warnings regardless of quiet mode.",
    "false_positive_risk": "low"
  },
  {
    "id": "F12",
    ...
    "title": "Exponential backoff calculation is O(n) loop instead of bit shift, and produces wrong values",
    ...
    "impact": "Negligible.",
    "suggestion": "Use `math.pow(2, attempt-1)` or `2^(attempt-1)` (Lua supports `^` operator) for clarity: `local delay = math.floor(2^(attempt-1))`.",
    "false_positive_risk": "high"
  },
  {
    "id": "F13",
    ...
    "title": "wget_is_busybox() spawns a subprocess and reads output but doesn't handle io.popen failure",
    ...
    "impact": "Low — conservative fallback is safe.",
    "suggestion": "Also check for `wget -V` as an alternative version flag for completeness.",
    "false_positive_risk": "high"
  },
  {
    "id": "F14",
    ...
    "title": "nc response check uses HTTP/1.x pattern but server may respond with HTTP/2 or malformed status",
    ...
    "impact": "Legitimate successful uploads may be counted as failures if the server uses non-standard HTTP response formatting.",
    "suggestion": "The pattern is reasonable. Consider also matching `HTTP/2 2` for HTTP/2 cleartext responses, though nc-based HTTP/2 is unlikely.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F15",
    ...
    "title": "config.retries validation allows retries=0 to pass if --retries 0 is given",
  ...
  },
  {
    ...
    "title": "Boundary collision check reads entire file into memory for files up to 64KB",
    ...
    "impact": "Increased memory usage on memory-constrained embedded systems when processing many small files.",
    "suggestion": "Either skip the boundary check entirely (the random boundary is sufficiently unique in practice) or do the boundary check during the streaming copy to avoid the double-read.",
    "false_positive_risk": "low"
  },
  {
    "id": "F17",
    ...
    "title": "api_endpoint has scan_id appended with urlencode but scan_id from server may already be URL-safe",
  ...
  },
  {
    "id": "F18",
    "severity": "MEDIUM",
    "location": "mktemp / ~line 100",
    "title": "Temp files created by mktemp are tracked globally but cleanup_temp_files clears the list — if called multiple times, already-deleted files are not re-deleted (harmless) but the list is cleared before all files are removed if an error occurs mid-cleanup",
    "description": "`cleanup_temp_files()` iterates `temp_files` and calls `os.remove()` on each, then sets `temp_files = {}`. If `os.remove()` fails (e.g., permission error), the loop continues and the list is still cleared. This means a failed removal is silently ignored and the temp file leaks. On embedded systems with limited /tmp space, leaked temp files (especially multipart body files that can be up to 2MB) could fill /tmp.",
    "impact": "Temp file leaks on embedded systems with limited storage.",
    "suggestion": "Log a warning when `os.remove()` returns false: `if not os.remove(path) then io.stderr:write('[warn] Could not remove temp file: ' .. path .. '\\n') end`.",
    "false_positive_risk": "low"
  },
  {
    "id": "F19",
    ...
    "title": "curl response file and error file are registered in temp_files but resp_file itself is also registered via mktemp — double registration",
    ...
    "impact": "Memory growth in the temp_files tracking table on large scans with many retries.",
    "suggestion": "Reuse a single resp_file per upload session (pass it in or create it once in submit_file), or clean up resp_file immediately after each upload attempt rather than deferring to cleanup_temp_files.",
    "false_positive_risk": "low"
  },
  {
    "id": "F20",
    ...
    "title": "sanitize_filename uses Lua pattern '[\"\\\\;]' which has incorrect escaping",
    ...
    "impact": "No functional impact; code is harder to read.",
    "suggestion": "Add a comment: `-- character class: double-quote, backslash, semicolon` to clarify intent.",
    "false_positive_risk": "high"
  },
  {
    "id": "F21",
    "severity": "LOW",
    "location": "send_collection_marker / ~line 460",
    "title": "nc is not used as a fallback for collection markers even though it's a supported upload tool",
    ...
    "impact": "On nc-only embedded systems, collection markers are silently dropped. The scan proceeds but the server cannot track collection start/end.",
    "suggestion": "Add an `elseif tool == 'nc' then` branch that uses the nc backend to POST the JSON body, similar to `upload_with_nc` but for JSON content-type. Or at minimum, log a warning: `log_msg('warn', 'Collection markers not supported with nc upload tool')`.",
    "false_positive_risk": "low"
  },
  {
    "id": "F22",
    ...
    "title": "api_endpoint query string separator logic has off-by-one: scan_id separator check uses api_endpoint which already has ?source= but the check is correct",
  ...
  },
  {
    ...
    "title": "math.randomseed not called before math.random usage in build_multipart_body",
    ...
    "impact": "Low — boundary collisions between concurrent collector instances could theoretically cause multipart parsing issues, but this is an edge case.",
    "suggestion": "Add `math.randomseed(os.time())` near the top of `main()` before any `math.random()` calls.",
    "false_positive_risk": "low"
  }
]
```