...
[
  {
    ...
    "title": "Duplicate code block causes scan_directory to never execute file processing",
  ...
  },
  {
    ...
    "title": "Second stray '>>>REPLACE' marker causes syntax error in main()",
  ...
  },
  {
    ...
    "title": "Boundary collision check misses the preamble and epilogue themselves",
    ...
    "impact": "A file whose first bytes happen to match the boundary string will produce a malformed multipart body, causing the server to reject the upload or misparse it. On embedded targets with limited retry budgets this means silent data loss.",
    "suggestion": "Also check for the plain '--' .. candidate pattern (without leading \\r\\n) to cover the opening delimiter:\n```lua\nlocal needle1 = \"\\r\\n--\" .. candidate\nlocal needle2 = \"--\" .. candidate  -- covers opening delimiter\nif search_buf:find(needle1, 1, true) or search_buf:find(needle2, 1, true) then\n    collision = true\n    break\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F4",
    ...
    "title": "nc upload silently succeeds on HTTP error responses that contain a 2xx line elsewhere",
    ...
    "impact": "Failed uploads are silently counted as successful; files are not retried and counters.files_failed is not incremented.",
    "suggestion": "Anchor the match to the start of the response:\n```lua\nif resp:match(\"^HTTP/1%.%d 2%d%d\") then return true end\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F5",
    ...
    "title": "Temp file list truncation uses table.remove in reverse but leaves holes after partial removal",
    ...
    "impact": "Temp files from failed attempts may not be deleted, leaking disk space on memory-constrained embedded devices. On a device with 2 MB RAM and a /tmp on tmpfs this can cause subsequent uploads to fail.",
    "suggestion": "Reset `temps_before` at the start of each retry iteration:\n```lua\nfor attempt = 1, config.retries do\n    local temps_before = #temp_files  -- capture inside loop\n    local success = false\n    -- ... upload ...\n    -- cleanup\n    for i = #temp_files, temps_before + 1, -1 do\n        os.remove(temp_files[i])\n        table.remove(temp_files, i)\n    end\n    if success then return true end\n    -- ...\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F6",
    ...
    "title": "Missing -X POST flag in curl upload command causes GET request",
  ...
  },
  {
    ...
    "title": "find -mtime predicate semantics: -mtime -N finds files modified less than N*24h ago, not N days",
    ...
    "impact": "Low operational impact; files slightly outside the expected window may be missed or included.",
    "suggestion": "This is acceptable behavior matching POSIX find semantics. Update the help text to say 'Max file age in days (uses find -mtime; -mtime -N finds files modified less than N×24h ago)'.",
  ...
  },
  {
    ...
    "title": "nc collection marker path uses #body (byte count) but body may contain multi-byte sequences",
    ...
    "impact": "Minimal; Content-Length will be correct.",
    "suggestion": "No code change needed; add a comment confirming that #body is byte-accurate in Lua 5.1.",
    "false_positive_risk": "high"
  },
  {
    "id": "F9",
    ...
    "title": "Pre-computed cloud pattern tables (_cloud_mid_patterns, _cloud_end_suffixes) are never used",
  ...
  },
  {
    ...
    "title": "nc upload URL path parsing drops query string when path_rest is nil",
    ...
    "impact": "If the endpoint URL is ever constructed without a path, nc uploads go to '/' instead of the correct API path, causing 404 errors.",
  ...
  },
  {
    ...
    "title": "sanitize_filename does not handle DEL character (0x7F) correctly via %c pattern",
    ...
    "impact": "Minimal; DEL in filenames is extremely rare. The double-replacement is harmless.",
    "suggestion": "Combine into a single pattern or add a comment explaining the belt-and-suspenders approach. No functional change needed.",
    "false_positive_risk": "high"
  },
  {
    "id": "F12",
    ...
    "title": "stats_json is concatenated into JSON body without validation, allowing injection",
    ...
    "impact": "Low risk currently since stats_json is always constructed from integer counters. If the function is ever called with external data, JSON injection is possible.",
    "suggestion": "Either document that stats_json must be a pre-validated JSON fragment, or validate that it contains only expected characters (digits, quotes, colons, braces, commas).",
    "false_positive_risk": "medium"
  },
  {
    "id": "F13",
    ...
    "title": "SSL + nc detection runs 'which nc' twice unnecessarily and logs misleading warning",
  ...
  },
  {
    ...
    "title": "total_body size calculation is incorrect: preamble length counted but file bytes not streamed to variable",
    ...
    "impact": "On I/O errors mid-file, wget uploads will have incorrect Content-Length, causing server-side parse failures. curl uses `--data-binary @file` which determines size from the file itself, so curl is unaffected.",
    "suggestion": "After writing the temp file, use `file_size_kb` or seek to determine the actual written size rather than computing it incrementally, or check for write errors:\n```lua\nlocal written, werr = out:write(chunk)\nif not written then\n    src:close(); out:close()\n    return nil, nil, nil\nend\n```",
    "false_positive_risk": "medium"
  },
  {
    "id": "F15",
    ...
    "title": "Octal escape decoding in /proc/mounts is incomplete: only \\040, \\011, \\012, \\134 handled",
    ...
    "impact": "Files on network or special filesystems with unusual characters in their mount point paths will not be excluded, potentially causing uploads of files from network shares or special filesystems.",
    "suggestion": "Use a general octal decoder:\n```lua\nmp = mp:gsub(\"\\\\(%d%d%d)\", function(oct)\n    return string.char(tonumber(oct, 8))\nend)\n```\nThis handles all octal escapes in a single pass.",
    "false_positive_risk": "low"
  },
  {
    "id": "F16",
    ...
    "title": "syslog logger command uses shell_quote for facility.priority but logger -p expects unquoted argument on some BusyBox versions",
    ...
    "impact": "Syslog messages may be dropped or logged with wrong priority on BusyBox systems.",
    "suggestion": "Since syslog_facility and prio are controlled internal values (not user input that needs quoting), use them directly after validating they contain only safe characters:\n```lua\nlocal prio_arg = config.syslog_facility .. \".\" .. prio\n-- prio_arg contains only [a-z0-9.] so no quoting needed\nos.execute(string.format(\"logger -p %s %s 2>/dev/null\",\n    prio_arg, shell_quote(\"thunderstorm-collector: \" .. clean)))\n```",
    "false_positive_risk": "medium"
  },
  {
    "id": "F17",
    ...
    "title": "scan_id appended to api_endpoint with incorrect separator detection",
    ...
    "impact": "When a source name is set, the scan_id is appended with '?' instead of '&', producing an invalid API endpoint URL. All file uploads after the begin marker will go to a malformed URL, likely causing 400 errors from the server.",
    "suggestion": "Use the plain=true flag for find:\n```lua\nlocal sep = \"&\"\nif not api_endpoint:find(\"?\", 1, true) then sep = \"?\" end\napi_endpoint = api_endpoint .. sep .. \"scan_id=\" .. urlencode(scan_id)\n```",
    "false_positive_risk": "low"
  }
]
```