...
[
  {
    ...
    "title": "nc upload reads entire file body into Lua memory before writing to request file",
    ...
    "impact": "On low-memory embedded targets (2-4 MB RAM), uploading files near the max_size_kb limit via nc will exhaust available memory, causing the Lua interpreter to crash or the upload to fail silently.",
    "suggestion": "Stream the body file to the request file in chunks instead of reading all at once:\n```lua\nlocal chunk_size = 8192\nwhile true do\n    local chunk = body_f:read(chunk_size)\n    if not chunk then break end\n    req_f:write(chunk)\nend\n```",
  ...
  },
  {
    ...
    "title": "Content-Length in nc upload is computed from preamble+epilogue+chunk-counted bytes but preamble/epilogue byte counts may be wrong on systems where Lua counts characters not bytes",
    ...
    "impact": "After careful analysis, the Content-Length computation is correct. Downgrading this finding.",
    "suggestion": "No change needed for Content-Length. However, ensure temp files created by mktemp() before a failure in build_multipart_body are still cleaned up (they are, via cleanup_temp_files at exit).",
  ...
  },
  {
    ...
    "title": "JSON body passed directly to curl -d via shell_quote — NUL bytes and very long source names could cause shell argument issues; more critically, body written to temp file for wget but passed as shell argument for curl",
    ...
    "impact": "On systems with small ARG_MAX (some embedded Linux kernels set it to 128KB or less), a large JSON body passed as a curl argument could cause 'Argument list too long' errors, silently failing the collection marker.",
    "suggestion": "Write the JSON body to a temp file for curl too, and use `--data-binary @file` instead of `-d shell_quote(body)`:\n```lua\nlocal body_tmp = mktemp()\nlocal bf = io.open(body_tmp, 'wb')\nif bf then bf:write(body); bf:close() end\nlocal cmd = string.format(\n    'curl -sS --fail -o %s %s%s-H %s --max-time 10 --data-binary @%s %s 2>/dev/null',\n    shell_quote(resp_file), insecure, ca_cert_flag,\n    shell_quote('Content-Type: application/json'),\n    shell_quote(body_tmp), shell_quote(url))\n```",
  ...
  },
  {
    ...
    "title": "curl --fail causes non-zero exit on 4xx/5xx but error body is read from resp_file which may be empty when --fail suppresses output",
    "description": "curl is invoked with `--fail --show-error`. When the server returns a 4xx or 5xx, curl with `--fail` exits non-zero AND does not write the response body to the output file (it discards it). The code then tries to read resp_file to check for `\"reason\"` in the response body. This check will never trigger on HTTP error responses when using --fail, because the body is discarded. The error is caught by exec_ok() returning false, but the specific server rejection message is lost.",
    "impact": "Server-side rejection messages (e.g., file type not accepted, quota exceeded) are silently lost. Operators cannot distinguish network failures from server-side rejections in logs.",
    "suggestion": "Remove `--fail` and instead check the HTTP status code manually, or use `--fail-with-body` (curl 7.76+, not available on all embedded targets). Alternatively, capture stderr for the HTTP error and parse the status from the response:\n```lua\n-- Remove --fail, add -w '%{http_code}' to a status file\nlocal cmd = string.format(\n    'curl -sS -X POST %s%s%s -H %s --data-binary @%s -o %s -w %%{http_code} 2>%s',\n    ...)\n-- Then check status code from output\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F5",
    ...
    "title": "find -mtime uses days but the semantics differ: -mtime -N means modified within N*24h, not N calendar days; also -mtime -0 would find nothing",
    ...
    "impact": "Users who set --max-age 0 expecting 'today's files' will get no results with no warning. The behavior is surprising.",
    "suggestion": "Either document that max_age=0 scans no files, or change the validation to require max_age >= 1, or use `-mtime -1` as minimum. Add a warning in validate_config:\n```lua\nif config.max_age == 0 then\n    log_msg('warn', 'max-age=0 will match no files (find -mtime -0 matches nothing)')\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F6",
    ...
    "title": "nc response check uses HTTP/1.x pattern but nc may receive partial response or no response before connection closes",
  ...
  },
  {
    ...
    "title": "sanitize_filename does not escape all characters that are special in multipart Content-Disposition headers",
    ...
    "impact": "Maliciously or accidentally named files with control characters in their names could corrupt the multipart body, causing the server to reject the upload or misparse the boundary, potentially leading to incorrect file association.",
    "suggestion": "Add control character sanitization to sanitize_filename:\n```lua\nfunction sanitize_filename(s)\n    if not s then return '' end\n    local r = s:gsub('[\"\\\\;]', '_')\n    r = r:gsub('%c', '_')  -- replace all control chars including \\r, \\n, \\t, etc.\n    return r\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F8",
    ...
    "title": "Mount point paths with spaces or special characters in /proc/mounts are not handled correctly",
    ...
    "impact": "Network or special filesystem mount points with spaces in their paths (e.g., `/mnt/my share`) would not be excluded from scanning. Files on network filesystems could be scanned and uploaded, causing performance issues or unintended data exfiltration.",
    "suggestion": "Decode \\040 escape sequences from the captured mountpoint:\n```lua\nlocal _, mp, fstype = line:match('^(%S+)%s+(%S+)%s+(%S+)')\nif mp then\n    mp = mp:gsub('\\\\040', ' ')  -- decode octal-escaped spaces\n    mp = mp:gsub('\\\\011', '\\t') -- decode octal-escaped tabs\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F9",
    ...
    "title": "Fallback temp file path using os.time() + math.random is predictable and has TOCTOU race",
    ...
    "impact": "On systems without mktemp, an attacker who can write to /tmp could pre-create the predictable temp file as a symlink, causing the script to write multipart bodies to attacker-controlled locations (symlink attack). This could lead to arbitrary file overwrite.",
    "suggestion": "Add math.randomseed(os.time()) at script startup (in main() before any mktemp calls). Also consider using os.clock() combined with os.time() for better entropy:\n```lua\nmath.randomseed(os.time() + math.floor(os.clock() * 1000000))\n```\nAdd this near the top of main() before any mktemp() calls.",
    "false_positive_risk": "low"
  },
  {
    "id": "F10",
    ...
    "title": "io.popen handle for find command is not closed on early return paths",
    ...
    "impact": "On embedded systems with low file descriptor limits (often 256 or fewer), leaked popen handles from errors in the processing loop could exhaust file descriptors, causing subsequent find commands or file opens to fail.",
    "suggestion": "Wrap the loop in pcall or use a local function with proper cleanup:\n```lua\nlocal ok, err = pcall(function()\n    for file_path in handle:lines() do\n        -- processing\n    end\nend)\nhandle:close()\nif not ok then\n    log_msg('error', 'Error during scan of ' .. dir .. ': ' .. tostring(err))\nend\n```",
    "false_positive_risk": "medium"
  },
  {
    "id": "F11",
    ...
    "title": "wget --post-file with multipart body does not set Content-Length header, relying on chunked transfer which BusyBox wget may not support",
    ...
    "impact": "Uploads via wget (especially BusyBox wget) to strict HTTP servers may fail silently or be rejected with 411 Length Required, causing files to be counted as failed uploads.",
    "suggestion": "Add an explicit Content-Length header to the wget command:\n```lua\nlocal cmd = string.format(\n    'wget -q -O %s %s%s--header=%s --header=%s --post-file=%s %s 2>/dev/null',\n    shell_quote(resp_file), insecure, ca_cert_flag,\n    shell_quote('Content-Type: multipart/form-data; boundary=' .. boundary),\n    shell_quote('Content-Length: ' .. tostring(body_len)),\n    shell_quote(body_file), shell_quote(endpoint))\n```",
    "false_positive_risk": "medium"
  },
  {
    "id": "F12",
    "severity": "MEDIUM",
    "location": "build_find_command / lines ~530-560",
    "title": "Shell injection risk if scan directory paths contain shell metacharacters",
    ...
    "impact": "After analysis, shell_quote correctly handles all cases including single quotes. Risk is low.",
    "suggestion": "No change needed. The shell_quote implementation is correct.",
    "false_positive_risk": "high"
  },
  {
    "id": "F13",
    "severity": "MEDIUM",
    "location": "detect_source_name / lines ~240-255",
    "title": "hostname -f may block for DNS timeout on systems with misconfigured DNS",
    "description": "`hostname -f` performs a DNS lookup to get the fully-qualified domain name. On embedded systems with misconfigured or unreachable DNS servers, this can block for 30-90 seconds (the DNS timeout). This happens at startup before any scanning begins.",
    "impact": "On embedded systems with broken DNS (common in isolated industrial networks), the script startup is delayed by 30-90 seconds waiting for hostname -f to time out before falling back to `hostname`.",
    "suggestion": "Reorder the commands to try `hostname` (no DNS lookup) before `hostname -f`, or add a timeout:\n```lua\nlocal cmds = {\n    'hostname 2>/dev/null',\n    'hostname -f 2>/dev/null',\n    'uname -n 2>/dev/null'\n}\n```\nOr use `timeout 2 hostname -f 2>/dev/null` if timeout is available.",
    "false_positive_risk": "low"
  },
  {
    "id": "F14",
    "severity": "MEDIUM",
    "location": "send_collection_marker / lines ~430-490",
    "title": "scan_id extracted from server response is not validated before use in URL construction",
    ...
    "impact": "After analysis, both urlencode and json_escape provide adequate protection. No actual vulnerability.",
    "suggestion": "Consider adding a length check on scan_id (e.g., reject if > 256 chars) as a defense-in-depth measure against unexpectedly large values from a compromised server.",
    "false_positive_risk": "high"
  },
  {
    "id": "F15",
    ...
    "title": "Progress reporting feature is advertised in --help and config but never implemented",
    ...
    "impact": "Users who enable --progress (e.g., for monitoring long-running scans) get no progress output. The feature is documented but silently does nothing, which is misleading.",
    "suggestion": "Either implement basic progress reporting in scan_directory (e.g., print a dot or counter every N files when config.progress is true), or remove the --progress/--no-progress flags and config field, and remove them from --help output to avoid misleading users.",
    "false_positive_risk": "low"
  },
  {
    "id": "F16",
    "severity": "MEDIUM",
    "location": "upload_with_curl / lines ~320-360",
    "title": "curl command missing --max-time / --connect-timeout flags for file uploads",
    ...
    "impact": "A single stalled TCP connection during file upload will hang the script indefinitely. On embedded systems that may have intermittent connectivity, this could cause the collector to never complete.",
    "suggestion": "Add appropriate timeouts to the upload curl command:\n```lua\nlocal cmd = string.format(\n    'curl -sS --fail --show-error -X POST %s%s%s' ..\n    ' --connect-timeout 30 --max-time 120' ..\n    ' -H %s --data-binary @%s -o %s 2>%s',\n    ...)\n```\nConsider making the timeout configurable.",
    "false_positive_risk": "low"
  },
  {
    "id": "F17",
    ...
    "title": "Cloud path detection suffix check uses hardcoded length that may be wrong for some entries",
    ...
    "impact": "Paths ending exactly with '/library/cloudstorage' (without a trailing slash) are not excluded. Files in macOS cloud storage directories accessed via network mounts would be scanned and uploaded.",
    "suggestion": "Fix the hardcoded length:\n```lua\nlocal cloud_suffix = '/library/cloudstorage'\nif lower:sub(-(#cloud_suffix)) == cloud_suffix then return true end\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F18",
    ...
    "title": "Syslog logging via os.execute('logger ...') spawns a shell process for every log message",
    ...
    "impact": "On embedded systems scanning directories with thousands of files with --syslog and --debug enabled, the overhead of spawning a logger process per log message could make the script 10-100x slower and could exhaust process table entries.",
    "suggestion": "Batch syslog messages or use a pipe to a persistent logger process. At minimum, add a check to skip syslog for debug-level messages unless explicitly needed:\n```lua\nif config.log_to_syslog and level ~= 'debug' then\n    -- spawn logger only for non-debug messages\nend\n```",
    "false_positive_risk": "low"
  },
  {
    "id": "F19",
    "severity": "MEDIUM",
    "location": "build_multipart_body / lines ~270-310",
    "title": "Temp files created by build_multipart_body accumulate throughout the run and are only cleaned at exit",
    ...
    "impact": "On embedded systems with small /tmp filesystems (often 1-4 MB tmpfs), accumulating thousands of temp files (even empty ones) will exhaust /tmp space, causing subsequent temp file creation to fail and uploads to fail.",
    "suggestion": "Clean up temp files after each file submission attempt rather than accumulating them. Track temp files per-upload and clean them immediately after the upload attempt:\n```lua\n-- After submit_file() returns, clean up files created during that submission\n-- Use a separate 'current_upload_temps' list that's cleared after each file\n```\nAlternatively, reuse temp file paths across uploads (create them once, reuse, delete at end).",
    "false_positive_risk": "low"
  },
  {
    "id": "F20",
    ...
    "title": "exec_capture does not check handle:close() return value for command exit status",
    ...
    "impact": "Low impact — the callers (detect_source_name, wget_is_busybox) handle empty/nil output correctly. No functional bug.",
    "suggestion": "Document that exec_capture returns output regardless of exit code, and callers should validate the output content rather than relying on exit status.",
    "false_positive_risk": "high"
  },
  {
    "id": "F21",
    ...
    "title": "validate_config allows retries=0 via the > 0 check but the submit_file loop uses 1-based indexing so retries=1 means one attempt with no retries",
    ...
    "impact": "Users who set --retries 3 expecting 3 retry attempts (4 total) get only 3 total attempts (2 retries). This is a documentation/UX issue rather than a crash.",
    "suggestion": "Either rename the parameter to 'attempts' and update help text, or change the loop to `for attempt = 1, config.retries + 1 do` to make retries mean the number of retry attempts after the first failure.",
    "false_positive_risk": "low"
  }
]
```