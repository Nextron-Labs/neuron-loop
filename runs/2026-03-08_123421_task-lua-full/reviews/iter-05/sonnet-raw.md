...
[
  {
    ...
    "title": "Dead code attempts to re-run find after output already consumed; find exit status never actually checked",
    ...
    "impact": "Permission errors during directory traversal are silently swallowed. The exit code will be 0 even when large portions of the filesystem were inaccessible. The `temp_files` table accumulates stale entries for every directory scanned, causing `cleanup_temp_files()` to attempt `os.remove()` on already-deleted paths (harmless but wasteful). On embedded systems with many scan directories this is a memory leak in the temp_files table.",
    "suggestion": "Either (a) use the wrapped sh -c approach that was in the original REPLACE block (write find status to a temp file, read it after popen closes) or (b) accept that popen exit status is unavailable in Lua 5.1 and remove the dead status-checking code entirely. Remove the mktemp()/os.remove() pair for status_file since it serves no purpose. If find exit status matters, use: `local wrapped = string.format('sh -c \\'%s; echo $? >%s\\'', cmd, status_file)` and open that file after `handle:close()`.",
  ...
  },
  {
    ...
    "title": "shell_quote applied to nc host and port arguments breaks nc invocation on BusyBox",
    ...
    "impact": "On a failed nc connection with a large file, the collector hangs for potentially thousands of seconds per retry, multiplied by `config.retries` directories × files. On embedded systems this can cause the collector to run indefinitely.",
    "suggestion": "Cap nc_timeout to a reasonable maximum: `local nc_timeout = math.min(300, math.max(30, 30 + math.floor(body_len / 10240)))`. Also add a sanity check that body_len is reasonable before computing the timeout.",
    "false_positive_risk": "low"
  },
  {
    "id": "F3",
    ...
    "title": "Boundary collision check reads file twice: once for collision check, once for body writing — TOCTOU and double I/O cost",
    ...
    "impact": "Double I/O on every file upload (significant on slow embedded flash). Race condition where file content changes between collision check and upload means the uploaded content may not match what was checked. Silent failure if file disappears between reads.",
    "suggestion": "Open the file once, read it into the temp file, then scan the temp file for boundary collisions (or generate the boundary first using /dev/urandom which makes collisions astronomically unlikely and skip the collision check entirely — a 128-bit random boundary has collision probability of ~2^-128 per file byte). If the collision check is kept, do it in a single pass while writing to the temp file.",
    "false_positive_risk": "low"
  },
  {
    "id": "F4",
    ...
    "title": "curl --data-binary @file sends the multipart body but Content-Length header is not set, relying on curl's chunked or server-determined length",
    ...
    "impact": "Server-side file rejections (e.g., file too large per server policy, unsupported format) are logged only as generic curl errors without the server's reason message, making debugging difficult.",
    "suggestion": "Remove `--fail` and instead check the HTTP status code separately, or use `--fail-with-body` (curl 7.76+, not available on embedded systems). Alternative: remove `--fail`, always read resp_file, check for HTTP error status in the response, and log the reason. Or keep `--fail` but also log the resp_file content when exec_ok returns false.",
    "false_positive_risk": "low"
  },
  {
    "id": "F5",
    "severity": "HIGH",
    "location": "send_collection_marker / stats_json validation",
    "title": "stats_json character whitelist rejects valid JSON characters, causing end marker to be sent without stats",
    "description": "The stats_json validation regex is `[^%w%s\":{},%.%[%]_%-]`. The stats string is built as: `'\"stats\":{\"scanned\":%d,...}'`. This contains only digits, colons, braces, quotes, commas, and letters — all whitelisted. However, the check `stats_json:match('[^%w%s\":{},%.%[%]_%-]')` uses Lua pattern matching where `%w` matches alphanumeric, `%s` matches whitespace. The double-quote character `\"` in a Lua pattern character class `[...]` is literal and should match. This appears correct. BUT: the stats string contains the key `\"elapsed_seconds\"` — the underscore is in the whitelist. This seems fine. The actual bug: the validation is applied to `stats_json` which is the caller-provided fragment. In `main()`, `stats` is constructed with `string.format` using `%d` format specifiers for all values, so it will only contain digits, letters, colons, braces, quotes, commas, and underscores. The validation will pass. However, if `elapsed` is negative (os.difftime returns negative if clock wraps or system time changes), `%d` will produce a minus sign `-` which IS in the whitelist. This is actually fine. The real issue is that the validation is security theater — the stats are entirely script-generated with no user input, so the validation adds complexity without benefit, and a future maintainer adding a string field to stats would silently have it stripped.",
    "impact": "Low immediate impact, but the validation creates a maintenance trap. More concretely: if the stats format ever includes a `+` sign or other valid JSON character not in the whitelist, stats will be silently omitted from the end marker, making server-side analytics incomplete.",
    "suggestion": "Since stats_json is entirely script-generated (not user input), remove the character validation entirely. If injection defense is desired, build the JSON object field by field using json_escape() on each value rather than validating a pre-built string.",
  ...
  },
  {
    ...
    "title": "The REPLACE block removes the find exit-status wrapper but the replacement code references the old `cmd` variable in dead status-checking code, creating confusion and a broken status check",
    ...
    "impact": "Maintainability issue that could lead a future developer to believe find errors are being caught when they are not. On embedded systems where permission errors are common (scanning /var, /home with limited privileges), this means partial scans are not flagged.",
    "suggestion": "Remove the entire dead status-checking block (from `-- Note: find exits non-zero...` to `os.remove(status_file)`). Add a comment: `-- Note: io.popen() in Lua 5.1 does not expose the child exit status; find permission errors are not detectable. Use the sh wrapper for exit-status-aware operation.`",
    "false_positive_risk": "low"
  },
  {
    "id": "F7",
    ...
    "title": "mktemp fallback uses exec_capture with sh -c mktemp, but if mktemp binary doesn't exist in sh's PATH either, die() is called — however the primary check already tried sh-accessible mktemp via exec_ok('which mktemp'), making the fallback redundant and the die() message misleading",
    ...
    "impact": "On systems without mktemp (some minimal BusyBox configurations), the collector dies immediately with a misleading error message. The `_urandom_suffix()` function exists but is never used for its intended purpose of providing a mktemp fallback.",
    "suggestion": "Add a pure-Lua fallback in mktemp(): if both mktemp attempts fail, generate a path using `_urandom_suffix()` and create the file with `io.open()`. Accept the TOCTOU risk with a comment, or use a loop to find an unused name. Fix the die() message to say 'mktemp not found or /tmp not writable'.",
    "false_positive_risk": "low"
  },
  {
    "id": "F8",
    ...
    "title": "nc response check anchors to start of string but HTTP/1.0 responses may have leading \\r\\n or server banners on some implementations",
    ...
    "impact": "Minor: on non-compliant servers that send leading whitespace before the status line, uploads via nc will always fail even when the server accepted the file.",
    "suggestion": "Change the match to `resp:match('HTTP/1[%.%d]+ 2%d%d')` without the `^` anchor, or use `resp:find('HTTP/1%.%d 2%d%d')` to be more lenient about leading content.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F9",
    ...
    "title": "find -mtime -N semantics: -mtime -14 finds files modified less than 14*24 hours ago, not 'within 14 days' as intended for max_age=14",
    ...
    "impact": "No actual bug here — behavior is as documented.",
    "suggestion": "No change needed. The -mtime semantics are correct.",
  ...
  },
  {
    ...
    "impact": "No actual bug in the backoff logic.",
    "suggestion": "No change needed for backoff. However, document that retries=1 means exactly one attempt (no retries), which may surprise users who expect retries=1 to mean 'retry once' (2 total attempts).",
    "false_positive_risk": "high"
  },
  {
    "id": "F11",
    ...
    "title": "sanitize_filename uses Lua pattern %c which in Lua 5.1 matches bytes 0x01-0x1F but NOT 0x00 (NUL byte), leaving NUL bytes in Content-Disposition filename",
    ...
    "impact": "Files with NUL bytes in their names (unusual but valid on Linux) would have malformed Content-Disposition headers, potentially causing server-side parse errors or security issues.",
    "suggestion": "Add explicit NUL handling: `r = r:gsub('%z', '_')` (Lua pattern `%z` matches NUL in Lua 5.1). Add this before or after the `%c` substitution.",
    "false_positive_risk": "low"
  },
  {
    "id": "F12",
    ...
    "title": "scan_id regex allows dots and hyphens but the character class is anchored incorrectly for UUIDs with uppercase hex",
    ...
    "impact": "If the server uses base64 or other encoding for scan_ids, begin and end markers will not be correlated on the server side, breaking collection tracking.",
    "suggestion": "Log a debug message when the response contains `scan_id` but the regex doesn't match: `if resp:find('\"scan_id\"') and id == nil then log_msg('debug', 'scan_id present in response but format not recognized') end`. Consider widening the character class to `[A-Za-z0-9_%.%-%+%/%=]+` to cover base64.",
    "false_positive_risk": "medium"
  },
  {
    "id": "F13",
    ...
    "title": "parse_proc_mounts only decodes \\040, \\011, \\012, \\134 but /proc/mounts can encode any byte as \\NNN octal",
    ...
    "impact": "Network/special filesystem mountpoints with unusual characters in their paths will not be excluded from scanning, potentially causing the collector to scan network filesystems (slow, potentially dangerous).",
    "suggestion": "Implement a general octal decoder: `mp = mp:gsub('\\\\(%d%d%d)', function(oct) return string.char(tonumber(oct, 8)) end)`. Apply this after the specific substitutions or replace them all with this general approach.",
    "false_positive_risk": "low"
  },
  {
    "id": "F14",
    ...
    "title": "wget --ca-certificate flag uses shell_quote but the = separator means the quoted value includes the = sign inside the quotes, which is wrong",
    ...
    "impact": "TLS certificate validation with custom CA bundles will fail when using wget, causing either TLS errors (if the server cert isn't trusted by the system CA store) or falling back to no validation. This defeats the --ca-cert security feature for wget users.",
    "suggestion": "Change to: `ca_cert_flag = shell_quote(\"--ca-certificate=\" .. config.ca_cert) .. \" \"` — quote the entire argument including the `=` and path. Or use: `\"--ca-certificate \" .. shell_quote(config.ca_cert) .. \" \"` if wget accepts space-separated form (it does for most options).",
    "false_positive_risk": "low"
  },
  {
    "id": "F15",
    ...
    "title": "scan_id appended to api_endpoint with urlencode but api_endpoint may already have a query string with source parameter, and the separator logic is wrong",
    ...
    "impact": "When `config.source` is empty (no source parameter), the scan_id is appended with `&` instead of `?`, producing a malformed URL like `http://server:8080/api/checkAsync&scan_id=...`. The server will likely reject this or ignore the scan_id parameter, breaking collection correlation.",
    "suggestion": "Change `api_endpoint:find(\"?\")` to `api_endpoint:find(\"?\", 1, true)` to use plain string matching. Same fix needed in any other place where literal `?` is searched in a URL.",
    "false_positive_risk": "low"
  },
  {
    "id": "F16",
    ...
    "title": "syslog logger command uses shell_quote for the message but the facility.priority argument is also shell_quoted — however the format string concatenates them without proper quoting of the combined argument",
    ...
    "impact": "Minimal — syslog_facility is hardcoded and not user-controllable via CLI.",
    "suggestion": "No immediate fix needed. If syslog_facility becomes user-configurable in the future, validate it against an allowlist of valid syslog facility names.",
    "false_positive_risk": "high"
  },
  {
    "id": "F17",
    ...
    "title": "total_body counter counts preamble bytes but file content bytes are counted as Lua string lengths which may differ from actual bytes written for binary files in text mode",
    ...
    "impact": "No impact on Linux/embedded targets. Would be a bug on Windows (CRLF translation) but that's not a target platform.",
    "suggestion": "No change needed for the target platform. Add a comment noting that binary mode is required for correct Content-Length calculation.",
    "false_positive_risk": "high"
  },
  {
    "id": "F18",
    ...
    "title": "detect_upload_tool calls 'which nc' twice: once to skip it for SSL, and once at the end to report a warning",
    ...
    "impact": "Two extra shell forks on embedded systems when nc is the only tool and SSL is required. Negligible in practice.",
    "suggestion": "Cache the nc availability: `local has_nc = exec_ok('which nc >/dev/null 2>&1')` at the top of the function, then use `has_nc` throughout.",
    "false_positive_risk": "low"
  },
  {
    "id": "F19",
    "severity": "LOW",
    "location": "main / summary double-print",
    "title": "Run summary is printed twice: once via log_msg('info', ...) and once directly to stderr",
    "description": "In main(), the summary is logged with `log_msg('info', ...)` which writes to stderr (unless quiet) and to the log file. Then immediately after, there's `if not config.quiet then